    ...
    public partial class UpdateService : ServiceBase {
        ...
        private void OnStartupPolicyPoll(object state) {
            ...
            if (!policyFound && !timedOut) {
                Logger.Debug("Startup policy poll attempt {0}: policy not yet in registry", _startupPollAttempts);
                return; // keep polling
            }
...
#pragma warning restore 1998 //This async method lacks 'await'
        ...
        private void CheckUpdate(object sender, ElapsedEventArgs e) {
            ...
            try {
                ...
                Logger.Debug("InstallationIsCritical check: publishDate={0} (UTC), threshold={1}, criticalAfter={2} (UTC)",
                    check.PublishDate, PolicySettings.EffectiveInstallationCritical(), check.PublishDate + PolicySettings.EffectiveInstallationCritical());
                if (InstallationIsCritical(check.PublishDate)) {
                    if (updatesDisabled) {
                        ...
                        Logger.Warn("Installation is critical! for ZDE version: {0}. update published at: {1} (UTC). approximate install time: {2} (local)", info.ZDEVersion, check.PublishDate, info.InstallTime);
                    ...
                    if (updatesDisabled) {
                        ...
                        Logger.Info("Installation reminder for ZDE version: {0}. update published at: {1} (UTC). approximate install time: {2} (local)", info.ZDEVersion, check.PublishDate, info.InstallTime);
        ...
        private DateTime InstallDateFromPublishDate(DateTime publishDate) {
            DateTime raw = publishDate + PolicySettings.EffectiveInstallationCritical();
            return SnapToMaintenanceWindow(raw);
        }

        private bool InstallationIsCritical(DateTime publishDate) {
            return DateTime.Now > publishDate + PolicySettings.EffectiveInstallationCritical();
        }
