...
namespace ZitiUpdateService.Utils {
    ...
    internal static class PolicySettings {
        ...
        private static void OnDebounceElapsed(object state) {
            ...
            // Re-arm the WMI watcher against the now-current key handle. WMI's
            // RegistryTreeChangeEvent subscription becomes stale if the watched
            // subtree is deleted and recreated (e.g. an admin re-applying GPO at
            // boot), so subsequent value writes silently stop firing events.
            // Calling StartWatching() here (StopWatching internally) rebinds the
            // subscription each time we know the registry state changed. Skipped
            // when the key is absent — Load() already called StopWatching in that
            // path, and RetryWatchIfNeeded will restart it on the next timer tick.
            if (HasPolicy) {
                StartWatching();
            }
        }
