...
header (step 12 tells you whether it is the regular or `-win32crypto` build), then in WinDbg `.exepath+ <dir
holding the exe>`, `.reload /f`, `.ecxr`, `kb`. **`!analyze -v` is not a substitute** -- its own stack walk
