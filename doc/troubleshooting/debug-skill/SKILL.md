...
grep -hoE "^\[[^]]+\].*load_tunnel_status_from_file\(\) Loading config file from .*" \
  /abs/path/capture/service/ziti-tunneler*.log*
grep -h "Restored old identity from the backup path\|Removing old identity from the backup path\|failed to copy backup identity file\|keeping existing file\|failed to remove backup file" \
  /abs/path/capture/service/ziti-tunneler*.log*
...
```bash
grep -h "Windows upgrade backup\|keeping existing file\|ignoring empty backup file\|failed to copy backup file\|failed to remove backup file\|failed restoring backup files" \
  /abs/path/capture/ZitiMonitorService/*.log
```
