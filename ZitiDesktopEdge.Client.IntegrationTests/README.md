...
- `ConnectAndStatusTests`: `DataClient` pipe connectivity and `GetStatus`
  round-trip.
- `IdentityLifecycleTests`: identity enrollment, enable/disable, removal,
  and state persistence across ziti service restart. State is observed
  through `GetStatus`.
