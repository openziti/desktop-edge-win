...
namespace ZitiDesktopEdge {
    ...
    public partial class IdentityDetails : UserControl {
        ...
        private void PopulateExternalProviders(IdentityDetails deets) {
            string gpoProvider = gpoPolicyViewModel.GpoDefaultExtAuthProvider;
            bool gpoLocked = gpoPolicyViewModel.IsDefaultExtAuthGpoLocked;
            ...
            if (gpoLocked && gpoProvider != null) {
                // auto-select the GPO-forced provider if it exists in the list
                for (int i = 0; i < deets.ProviderList.Items.Count; i++) {
                    string item = NormalizeProvider(deets.ProviderList.Items[i] as string);
                    if (string.Equals(item, gpoProvider, StringComparison.OrdinalIgnoreCase)) {
        ...
        private void IsDefaultProvider_Checked(object sender, RoutedEventArgs e) {
            if (gpoPolicyViewModel.IsDefaultExtAuthGpoLocked) return;
            string selectedProvider = ProviderList.SelectedItem.ToString();
        ...
        private void IsDefaultProvider_PreviewMouseDown(object sender, MouseButtonEventArgs e) {
            if (gpoPolicyViewModel.IsDefaultExtAuthGpoLocked) {
                e.Handled = true;
                return;
            }
            if (ProviderList.SelectedItem == null) {
